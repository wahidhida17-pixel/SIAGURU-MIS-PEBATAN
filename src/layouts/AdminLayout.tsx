...
import { 
  ...
  Calendar, ClipboardList, Activity, Settings, 
  User, LogOut, Menu, X, CheckSquare, FileText, Award,
  FolderOpen, CalendarDays, Bell, Palette
} from 'lucide-react';
...
export const AdminLayout: React.FC = () => {
  ...
  const navigation = [
    ...
    { name: 'Guru', href: '/admin/guru', icon: UserSquare2 },
    ...
    { name: 'Penugasan Guru', href: '/admin/penugasan', icon: CheckSquare },
    { name: 'Jadwal Pelajaran', href: '/admin/jadwal', icon: Calendar },
    { name: 'Absensi Siswa', href: '/admin/absensi', icon: Calendar },
    { name: 'Jurnal Mengajar', href: '/admin/jurnal', icon: FileText },
    { name: 'Penilaian Siswa', href: '/admin/assessment', icon: Activity },
    { name: 'Rapor & Leger', href: '/admin/rapor', icon: Award },
    { name: 'Dokumen Madrasah', href: '/admin/documents', icon: FolderOpen },
    ...
    <div className="min-h-screen bg-[#F0F4F1] dark:bg-slate-950 flex font-sans text-slate-800 dark:text-slate-100 transition-colors">
      ...
      <aside className={`hidden lg:flex w-64 flex-col ${sidebarBgClass} h-screen sticky top-0 shrink-0 shadow-2xl transition-all border-r border-white/5`}>
        ...
        <div className="p-4 border-t border-white/10 space-y-2">
          <div className="bg-black/20 backdrop-blur-sm p-3 rounded-xl flex items-center gap-3 border border-white/5">
            <Avatar 
              fallback={userProfile?.displayName?.charAt(0) || 'A'} 
              src={userProfile?.photoURL} 
              className="w-8 h-8 font-bold text-xs shadow-sm"
              style={{ backgroundColor: currentThemeOption.accentHex }}
            />
            <div className="overflow-hidden min-w-0 flex-1">
              <p className="text-xs font-bold text-white truncate">{userProfile?.displayName || 'Administrator'}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-16 lg:pb-0">
        ...
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 lg:px-8 flex items-center justify-between shrink-0 shadow-sm sticky top-0 z-30 transition-colors">
          ...
          <div className="hidden lg:block">
            ...
            <h2 className="font-bold text-slate-800 dark:text-slate-100 text-lg">Dashboard Admin</h2>
          </div>
          ...
          <div className="flex items-center gap-2.5 sm:gap-4">
            ...
            {/* Notification Bell */}
            <NotificationBellDropdown currentUser={userProfile ? { uid: userProfile.uid, name: userProfile.displayName || userProfile.email || 'Admin', role: 'admin' } : undefined} />
      ...
      <div className="lg:hidden fixed bottom-0 w-full bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 z-40 transition-colors">
        <div className="flex justify-around py-1">
          <Link to="/admin/dashboard" className="flex flex-col items-center py-1.5 px-3 text-slate-500 dark:text-slate-400 hover:text-emerald-600">
            <LayoutDashboard className="h-5 w-5" />
            <span className="text-[10px] mt-0.5">Home</span>
          </Link>
          <Link to="/admin/guru" className="flex flex-col items-center py-1.5 px-3 text-slate-500 dark:text-slate-400 hover:text-emerald-600">
            <UserSquare2 className="h-5 w-5" />
            <span className="text-[10px] mt-0.5">Guru</span>
          </Link>
          <Link to="/admin/siswa" className="flex flex-col items-center py-1.5 px-3 text-slate-500 dark:text-slate-400 hover:text-emerald-600">
          ...
          <div className="fixed inset-y-0 right-0 w-72 bg-white dark:bg-slate-900 shadow-2xl flex flex-col border-l border-slate-200 dark:border-slate-800 transition-colors">
            <div className="p-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2.5">
                <SchoolLogo size="sm" variant="circle" />
                <h2 className="text-lg font-bold text-slate-800 dark:text-white tracking-tight">Menu Admin</h2>
              </div>
