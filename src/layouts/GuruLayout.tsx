...
import { 
  LayoutDashboard, Clock, FileText, Calendar, 
  ClipboardList, Activity, Bot, User, LogOut, Menu, X, Award,
...
export const GuruLayout: React.FC = () => {
  ...
  if (role !== 'guru') return <Navigate to="/unauthorized" replace />;

  const navigation = [
    { name: 'Dashboard', href: '/guru/dashboard', icon: LayoutDashboard },
    { name: 'Jadwal Saya', href: '/guru/jadwal', icon: Clock },
    { name: 'Presensi Siswa', href: '/guru/absensi', icon: Calendar },
    { name: 'Jurnal Mengajar', href: '/guru/jurnal', icon: FileText },
    { name: 'Nilai Siswa', href: '/guru/nilai', icon: Activity },
    { name: 'Rapor Siswa', href: '/guru/rapor', icon: Award },
    { name: 'Perangkat Ajar', href: '/guru/administrasi', icon: ClipboardList },
    { name: 'Dokumen Guru', href: '/guru/documents', icon: FolderOpen },
    { name: 'Kalender & Agenda', href: '/guru/calendar', icon: CalendarDays },
    { name: 'AI Guru', href: '/guru/ai', icon: Bot },
    { name: 'Profil', href: '/guru/profile', icon: User },
  ];
    ...
    <div className="min-h-screen bg-[#F0F4F1] dark:bg-slate-950 flex font-sans text-slate-800 dark:text-slate-100 transition-colors">
      ...
      <aside className={`hidden lg:flex w-64 flex-col ${sidebarBgClass} h-screen sticky top-0 shrink-0 shadow-2xl transition-all border-r border-white/5`}>
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/70 text-[10px] uppercase font-bold tracking-wider">MI Syuriyah Pebatan</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-white/10 space-y-2">
          <div className="bg-black/20 backdrop-blur-sm p-3 rounded-xl flex items-center gap-3 border border-white/5">
            <Avatar 
              fallback={userProfile?.displayName?.charAt(0) || 'G'} 
              src={userProfile?.photoURL} 
              className="w-8 h-8 font-bold text-xs shadow-sm"
              style={{ backgroundColor: currentThemeOption.accentHex }}
            />
            <div className="overflow-hidden min-w-0 flex-1">
              <p className="text-xs font-bold text-white truncate">{userProfile?.displayName || 'Guru Madrasah'}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-16 lg:pb-0">
        ...
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 lg:px-8 flex items-center justify-between shrink-0 shadow-sm sticky top-0 z-30 transition-colors">
          ...
          <div className="hidden lg:block">
            <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">SIAGURU MI SYURIYAH PEBATAN</p>
            <h2 className="font-bold text-slate-800 dark:text-slate-100 text-lg">Dashboard Guru</h2>
          </div>
          ...
          <div className="flex items-center gap-2.5 sm:gap-4">
            ...
            {/* Notification Bell */}
            <NotificationBellDropdown currentUser={userProfile ? { uid: userProfile.uid, name: userProfile.displayName || userProfile.email || 'Guru', role: 'guru' } : undefined} />
      ...
      <div className="lg:hidden fixed bottom-0 w-full bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 z-40 transition-colors">
        <div className="flex justify-around py-1">
          <Link to="/guru/dashboard" className="flex flex-col items-center py-1.5 px-3 text-slate-500 dark:text-slate-400 hover:text-emerald-600">
            <LayoutDashboard className="h-5 w-5" />
            <span className="text-[10px] mt-0.5">Home</span>
          </Link>
          <Link to="/guru/jadwal" className="flex flex-col items-center py-1.5 px-3 text-slate-500 dark:text-slate-400 hover:text-emerald-600">
            <Clock className="h-5 w-5" />
            <span className="text-[10px] mt-0.5">Jadwal</span>
          </Link>
          <Link to="/guru/absensi" className="flex flex-col items-center py-1.5 px-3 text-slate-500 dark:text-slate-400 hover:text-emerald-600">
          ...
          <div className="fixed inset-y-0 right-0 w-72 bg-white dark:bg-slate-900 shadow-2xl flex flex-col border-l border-slate-200 dark:border-slate-800 transition-colors">
            <div className="p-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2.5">
                <SchoolLogo size="sm" variant="circle" />
                <h2 className="text-lg font-bold text-slate-800 dark:text-white tracking-tight">Menu Guru</h2>
              </div>
