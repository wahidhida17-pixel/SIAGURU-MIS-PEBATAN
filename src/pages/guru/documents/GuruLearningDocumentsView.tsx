...
export const GuruLearningDocumentsView: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Filter learning relevant categories
      ...
      const filtered = docs.filter(
        d =>
          learningCategories.some(c => d.category.includes(c)) ||
          d.tags?.some(t => learningCategories.some(c => t.toLowerCase().includes(c.toLowerCase())))
      );
      setDocuments(filtered);
    } catch (e) {
      console.error('Error loading learning docs:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <LoadingSpinner />
        <p className="text-xs text-slate-500 mt-2">Memuat perangkat pembelajaran...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Perangkat Kurikulum Merdeka: Prota, Promes, ATP, Modul Ajar, KKTP, Bahan Ajar, dan LKPD
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Link
            to="/guru/perangkat/bank"
          ...
          >
            Bank Perangkat <ArrowUpRight className="w-3.5 h-3.5" />
          </Link>
          <button
      ...
      {filteredDocs.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center space-y-3">
          ...
          <h3 className="font-bold text-sm text-slate-800 dark:text-slate-200">
            Belum ada berkas dokumen pembelajaran
          </h3>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Anda dapat mengunggah file Modul Ajar / Prota / Promes atau menyalin langsung dari template resmi.
          </p>
          <button
            onClick={() => setIsUploaderOpen(true)}
            className="px-4 py-2 rounded-lg bg-emerald-600 text-white text-xs font-semibold inline-flex items-center gap-1.5"
          >
            <Upload className="w-4 h-4" /> Upload Perangkat Ajar
          </button>
        </div>
      ) : (
